		     ...
		     void (*post)(job *p, int e)) /* post action func */
{
		...
		if (**pargs == '%') {
			if (strcmp(*pargs + 1, "jobid") == 0) {
				...
				if (pjob->ji_wattr[(int) JOB_ATR_altid].at_val.at_str) {
					strncpy(buf, pjob->ji_wattr[(int) JOB_ATR_altid].at_val.at_str, sizeof(buf) - 1);
					buf[sizeof(buf) - 1] = '\0';
				} else
					strcpy(buf, "NULL");
	...
	if ((child = fork_me(-1)) == 0) { /* child */
		...
		if (set_mach_vars(pjob, &vtable) != 0) {
			log_event(PBSEVENT_JOB | PBSEVENT_SECURITY, PBS_EVENTCLASS_JOB, LOG_ERR, pjob->ji_qs.ji_jobid, "failed to setup dependent environment!");
			free(args);
			return -1;
		}
		...
		if (transmog) {
			 ...
			 * being restarted.
			 */
			memset(pjob->ji_extended.ji_ext.ji_4jid, 0, sizeof(pjob->ji_extended.ji_ext.ji_4jid));
#endif
			j = set_job(pjob, &sjr);
			if (j < 0) {
				if (j == -1)
