...
req_quejob(struct batch_request *preq)
{
	...
	while (psatl) {
		...
		if (!strcasecmp(psatl->al_name, ATTR_executable) && ((psatl->al_value != NULL) && (psatl->al_value[0] != '\0'))) {
			job_executable = 1;
		}
	...
	if (preq->rq_fromsvr) {
		...
		if (i == 0) {	/* Normal job */
			(void)sprintf(jidbuf, "%lld.%s", next_svr_sequence_id, server_name);
		} else {	/* Array Job */
			(void)sprintf(jidbuf, "%lld[].%s", next_svr_sequence_id, server_name);
		}
		jid = jidbuf;
...
req_commit_now(struct batch_request *preq, job *pj)
{
	...
	pj->ji_qs.ji_state = JOB_STATE_TRANSIT;
	pj->ji_wattr[(int) JOB_ATR_state].at_val.at_char = 'T';
	pj->ji_wattr[(int) JOB_ATR_state].at_flags |= ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
	pj->ji_qs.ji_substate = JOB_SUBSTATE_TRANSICM;
	pj->ji_wattr[(int) JOB_ATR_substate].at_flags |= ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
	...
	pj->ji_wattr[(int)JOB_ATR_qrank].at_flags |= ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
	...
	if ((rc = svr_enquejob(pj)) != 0) {
		if (rc == PBSE_JOBEXIST) {
			...
			if ((svr_chk_history_conf()) && ((pj_exist = find_job_avl(pj->ji_qs.ji_jobid)) != NULL)) {
				...
				* exists.
				*/
				if ((pj_exist->ji_qs.ji_state == JOB_STATE_MOVED)) {
					job_purge(pj_exist);
					goto retry;
				}
			}
			job_free(pj); /* use job_free() instead of job_purge() to not delete the job that has the clash */
		} else {
			job_purge(pj);
		}
