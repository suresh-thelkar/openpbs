...
#ifndef PBS_MOM
	pj = find_job_avl(jobid);
	/* first check avl tree, if not found, load from DB */
	if (pj == NULL) {
		if ((pj = job_recov_db(jobid, NULL))) {
			attribute *pattr = &(pj->ji_wattr[JOB_ATR_server_index]);
			if ((pattr->at_flags & ATR_VFLAG_SET) && (pattr->at_val.at_long == myindex)) {
				pbsd_init_job(pj, 1);
			} else {
