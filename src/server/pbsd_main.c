...
static time_t last_obj_autorefresh_tm = -1;
...
try_db_again:
	...
	if (server_init_type != RECOV_CREATE) {
		 ...
		 * and fairshare usage.
		 */
		(void)contact_sched(SCH_CONFIGURE, NULL,  dflt_scheduler, CONN_SCHED_PRIMARY);
	}
	...
	while ((*state != SV_STATE_DOWN) && (*state != SV_STATE_SECIDLE)) {
		...
		/* 
		 * keep track of a pseudo server iteration id
		 * We will base it on a simple server iteration number
		 * which needs to be only different in every server iteration
		 */
		UPDATE_SVR_TRX();
		...
		/* 
		 * periodically, or once per server iteration, load the common objects like server, sched, queues 
		 * Note: This does not perclude loading any changed queue or server or sched object when a find_object() 
		 * function is called, which should automatically try to reload it anyway
		 * 
		 * Deleted objects will also be "noticed" in this refresh
		 * 
		 */ 
		if ((last_obj_autorefresh_tm == -1) || ((time(0) - last_obj_autorefresh_tm) > OBJ_AUTOREFRESH_INTERVAL)) {
			...
			last_obj_autorefresh_tm = time(0);
		}
