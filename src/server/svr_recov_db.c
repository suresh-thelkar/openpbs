...
sched_recov_db(void)
{
	...
	while ((rc = pbs_db_cursor_next(conn, state, &obj)) == 0) {
		...
		if (!strncmp(dbsched.sched_name, DEFAULT_SCHED_NAME,
				strlen(DEFAULT_SCHED_NAME))) {
			dflt_scheduler = psched;
		}
		db_to_svr_sched(psched, &dbsched);

		psched->sch_attr[(int)SCHED_ATR_sched_cycle_len].at_val.at_long =
		...
		if (pbs_conf.pbs_use_tcp == 0) {
			...
			index = find_attr(sched_attr_def, ATTR_throughput_mode,
					SCHED_ATR_LAST);
			psched->sch_attr[index].at_flags = 0;
		}
	}
	pbs_db_cursor_close(conn, state);
	if (pbs_db_end_trx(conn, PBS_DB_COMMIT) != 0)
