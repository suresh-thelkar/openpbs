...
confirm_reservation(status *policy, int pbs_sd, resource_resv *unconf_resv, server_info *nsinfo)
{
	...
	if (confirmd_occr == occr_count) {
		...
		pbsrc = pbs_confirmresv(get_svr_inst_fd(pbs_sd, nresv->svr_inst_id), nresv_parent->name, short_xc,
		 ...
		 */
		pbsrc = pbs_confirmresv(get_svr_inst_fd(pbs_sd, nresv->svr_inst_id), nresv_parent->name, const_cast<char *>("null"),
