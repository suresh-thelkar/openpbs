...
close_server_conns(int index_to_shards)
{
	...
	if (get_max_servers() > 1) {
		...
		if (svr_sock_pair.ch_socket >= 0)
			close_tcp_connection(svr_sock_pair.ch_socket);
...
main(int argc, char *argv[])
{
	...
	for (shard_inst_idx = 0; shard_inst_idx < num_cfg_svrs; shard_inst_idx++) {
		if (pbs_conf.psi[shard_inst_idx]->name != NULL)
			addclient(pbs_conf.psi[shard_inst_idx]->name);
	}
	...
	for (go=1; go;) {
		...
		if (schedule_wrapper(max_svrs, num_cfg_svrs, &update_svr,
				&read_fdset, opt_no_restart) == 1) {
			go = 0;
		}
	}
...
initialise_svr_sock_context(int max_svrs)
{
	if (max_svrs > 1) {
		/* Using server_sock as virtual fd */
                ...
                return set_conn_shards_exist(server_sock);
	} else {
		svr_sock_pair.ch_socket= -1;
		svr_sock_pair.ch_secondary_socket = -1;
	}
 ...
 */
static int
schedule_wrapper(int max_svrs, int num_cfg_svrs, int *update_svr,
		fd_set *read_fdset, int opt_no_restart)
{
	int shard_inst_idx;
	int sock_to_check  = -1;
	int cmd;
	...
	for (shard_inst_idx = 0; shard_inst_idx < num_cfg_svrs; shard_inst_idx++) {
		...
		if ((sock_to_check != -1) && FD_ISSET(sock_to_check, read_fdset)) {
			...
			if (ret != 1) {
				close_server_conns(shard_inst_idx);
				log_eventf(PBSEVENT_SYSTEM, PBS_EVENTCLASS_SCHED, LOG_ERR, __func__,
