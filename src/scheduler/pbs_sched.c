...
accept_client(int *max_sd)
{
	...
	if (socket_to_conn(new_socket, i) == -1) {
		CS_close_socket(new_socket);
		close(new_socket);
		return SCH_ERROR;
	}
...
main(int argc, char *argv[])
{
	...
	if (initialise_svr_sock_context() == -1) {
	...
	for (go=1; go;) {
		...
		for (index_to_shards = 0; index_to_shards < num_cfg_svrs; index_to_shards++) {
			if (max_svrs > 1) {
				sock_to_check = shard_conn[index_to_shards]->sd;
				second_connection = shard_conn[index_to_shards]->secondary_sd;
			} else {
			...
			if ((sock_to_check != -1) && FD_ISSET(sock_to_check, &read_fdset)) {
				...
				if (ret != 1) {
					close_server_conns(index_to_shards);
				...
				/* Keep track of time to use in SIGSEGV handler */
					...
					if (sched_ret != 0 )  /* magic happens here */ {
						close_server_conns(index_to_shards);
						if (cmd == SCH_QUIT) {
							go = 0;
							break;
